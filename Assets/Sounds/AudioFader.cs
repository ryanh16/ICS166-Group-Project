...
public static class AudioFader
{
    ...
    public static IEnumerator ChangeSound(AudioSource audioSource, AudioClip newClip, float t, float newVolume = 0f)
    {
        float startVolume = audioSource.volume;
        float endVolume = (newVolume<=0f)?startVolume:newVolume; 

        while (audioSource.volume > 0)
        {
            audioSource.volume -= startVolume * Time.deltaTime * 2 / t;
        ...
        while (audioSource.volume < endVolume)
        {
            audioSource.volume += endVolume * Time.deltaTime * 2 / t;
